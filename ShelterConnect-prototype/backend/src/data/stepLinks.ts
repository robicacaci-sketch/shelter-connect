/**
 * stepLinks.ts
 * Maps keywords found in roadmap step titles/descriptions to curated agency
 * links, so case managers can complete each step online rather than by phone.
 */
...
/**
 * Given the action title (and optionally the full description) of a roadmap step,
 * returns the best matching OnlineOption or null if no match found.
 */
export function getOnlineOptionForStep(
  actionTitle: string,
  description?: string
): OnlineOption | null {
  const haystack = `${actionTitle} ${description ?? ""}`.toLowerCase();
